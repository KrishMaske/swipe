...
async def save_user_cards_endpoint(request: SaveWalletCardsRequest, context: dict = Depends(get_user_context)):
...
async def nearby_merchants_endpoint(lat: float, lon: float, context: dict = Depends(get_user_context)):
    # get_nearby_merchants is now async
    return await get_nearby_merchants(lat, lon)


@router.post("/api/location/evaluate")
async def evaluate_location_endpoint(request: LocationEvaluateRequest, context: dict = Depends(get_user_context)):
    saved_cards = get_saved_user_cards(context)
    # resolve_place_details is now async
    place = await resolve_place_details(request.latitude, request.longitude)
